 ...
 * 
 * ClassName: HttpsSoapServer 
 *
 * Description:https soap
 * 
 * Copyright: Copyright (c) 2015年8月17日 下午11:06:09 
 * 
 ...
 * @version 1.0
 */
public class HttpsSoapServer {
public static Logger logger = LoggerFactory.getLogger(HttpsSoapServer.class);
...
public static void main(String[] args){
	...
	//初始化配置参数
	 List<NettyService> confList = server.initServer();
		for(NettyService nettyService : confList){
			if(PROTOCOL_HTTPS.equals(nettyService.getProtocol()) && FORMAT_SOAP.equals(nettyService.getFormat())){
	...
	//启动服务
	server.run(PROTOCOL_HTTPS,FORMAT_SOAP,Integer.parseInt(ret.getPort()),ret.getUri());
}
}
